...
pub fn benchmark_json_parsing(iterations: usize) -> (u64, u64, usize, usize) {
    ...
    println!("@bookTicker: {} ns/msg ({} bytes)", book_ticker_ns, book_ticker_json.len());
    println!("@depth:      {} ns/msg ({} bytes)", depth_ns, depth_json.len());
    
    (book_ticker_ns, depth_ns, book_ticker_json.len(), depth_json.len())
}
...
impl LatencyStats {
    ...
    fn print_report(&self) {
        ...
        println!("\n{}", format!("=== {} {} ===", self.symbol.to_uppercase(), self.stream_type.to_uppercase()).cyan().bold());
        ...
        println!("  P50:    {:>8.2} ms (network) + {:.3} µs (parsing) = {:.2} ms", 
                p50, parse_mean / 1000.0, p50 + parse_mean / 1_000_000.0);
        println!("  P99:    {:>8.2} ms (network) + {:.3} µs (parsing) = {:.2} ms", 
                p99, parse_mean / 1000.0, p99 + parse_mean / 1_000_000.0);
    }
}

async fn measure_combined_streams(symbol: &str, streams: Vec<&str>, duration_secs: u64) -> Vec<LatencyStats> {
    // Build combined stream URL
    let stream_params: Vec<String> = streams.iter()
    ...
    for stream in &streams {
        stats_map.insert(stream.to_string(), LatencyStats::new(stream.to_string(), symbol.to_string()));
    }
    
    let clock = Clock::new();
    let end_time = Instant::now() + Duration::from_secs(duration_secs);
    
    println!("Collecting data for {} seconds on combined stream...", duration_secs);
            ...
            Ok(Some(Ok(Message::Text(text)))) => {
                ...
                if let Ok(wrapper) = serde_json::from_str::<serde_json::Value>(&text) {
                    if let Some(data) = wrapper.get("data") {
                        ...
                        let network_latency_ms = (receive_time_ms as i64 - event_time_ms as i64).abs() as f64;
    ...
    println!("\nData collection complete. Total messages: {}", total_messages);
...
pub async fn run_production_benchmark() {
    ...
    println!("{}", "WEBSOCKET+JSON PRODUCTION LATENCY BENCHMARK".bright_white().bold());
    println!("{}", "Measuring real-time latency from Binance WebSocket streams".bright_white());
    println!("{}", "=".repeat(80).bright_blue());
    ...
    println!("{}", "LATENCY REPORT (COMBINED STREAM)".bright_white().bold());
    println!("{}", "=".repeat(80).bright_blue());
    ...
    println!("1. {} - Can reduce 100ms+ to 1-5ms", "Colocate near exchange".bright_yellow());
    println!("2. {} - Optimize network path and kernel", "Network tuning".bright_yellow());
    println!("3. {} - SBE/FIX saves ~15µs (minimal impact)", "Binary protocols".bright_yellow());
