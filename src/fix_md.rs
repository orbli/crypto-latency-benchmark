...
impl FixSession {
    ...
    fn sign_message(&self, payload: &str) -> String {
        ...
        if let Some(pos) = key_data.windows(2).position(|w| w[0] == 0x04 && w[1] == 0x20) {
            if key_data.len() >= pos + 2 + 32 {
                if let Ok(key_pair) = Ed25519KeyPair::from_seed_unchecked(&key_data[pos + 2..pos + 34]) {
                    let signature = key_pair.sign(payload.as_bytes());
                    return base64::encode(signature.as_ref());
                }
            }
        ...
        eprintln!("Key data length after PEM parsing: {} bytes", key_data.len());
        String::new()
    }
    ...
    pub fn build_logon(&mut self) -> String {
        ...
        let payload = format!("A{}{}{}{}{}{}{}{}",
            SOH,
            self.sender_comp_id, SOH,
            self.target_comp_id, SOH, 
            self.msg_seq_num, SOH,
            timestamp
        );
...
async fn connect_fix_tls(endpoint: &str) -> Result<tokio_rustls::client::TlsStream<TcpStream>, Box<dyn std::error::Error + Send + Sync>> {
    // Parse endpoint
...
async fn run_fix_session(api_key: String, private_key: Vec<u8>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let endpoint = "fix-md.binance.com:9000";
    
    println!("Establishing TCP+TLS connection to {}...", endpoint.bright_white());
        ...
        match tokio::time::timeout(Duration::from_secs(5), stream.read(&mut buffer)).await {
            Ok(Ok(n)) if n > 0 => {
                ...
                while let Some(start_pos) = accumulated.windows(5).position(|w| w == b"8=FIX") {
                    ...
                    if let Some(checksum_pos) = accumulated[start_pos..].windows(3).position(|w| w[0] == b'1' && w[1] == b'0' && w[2] == b'=') {
                        // Find end of checksum (next SOH after 10=)
                        let checksum_start = start_pos + checksum_pos;
                        if let Some(end_offset) = accumulated[checksum_start + 3..].iter().position(|&b| b == SOH_BYTE) {
                            let msg_end = checksum_start + 3 + end_offset + 1;
                            let fix_message = String::from_utf8_lossy(&accumulated[start_pos..msg_end]);
                            ...
                            if message_count <= 5 {
                                if let Some(msg_type) = fields.get(&TAG_MSG_TYPE) {
                                    ...
                                    println!("\n  Received FIX message type: {} ({})", msg_type, msg_type_name);
                                    
                                    // Show reject details if it's a reject message
                                    if msg_type == "3" {
                                        if let Some(text) = fields.get(&58) {  // Tag 58 is Text field
                                            println!("  Reject reason: {}", text);
                                        }
                                        if let Some(ref_seq) = fields.get(&45) {  // Tag 45 is RefSeqNum
                                            println!("  Rejected message seq: {}", ref_seq);
                                        }
                                        if let Some(ref_tag) = fields.get(&371) {  // Tag 371 is RefTagID
                                            println!("  Problem with tag: {}", ref_tag);
                                        }
                                        if let Some(ref_msg_type) = fields.get(&372) {  // Tag 372 is RefMsgType
            ...
            Err(_) => {
                ...
                let heartbeat_msg = format!("8=FIX.4.4{}35=0{}49={}{}56={}{}34={}{}52={}{}",
                    SOH, SOH, session.sender_comp_id, SOH, session.target_comp_id, 
                    SOH, session.msg_seq_num, SOH, session.get_timestamp(), SOH);
    ...
    // Send Logout
    println!("\nSending FIX Logout...");
    let logout = format!("8=FIX.4.4{}35=5{}34={}{}52={}{}10=000{}",
        SOH, SOH, session.msg_seq_num, SOH, session.get_timestamp(), SOH, SOH);
    ...
    if !parsing_latencies.is_empty() {
        ...
        println!("  {:.1}M messages/second parsing capacity", throughput / 1_000_000.0);
    } else {
...
pub async fn run_fix_production_benchmark() {
    ...
    println!("{}", "FIX MARKET DATA PRODUCTION BENCHMARK".bright_white().bold());
    println!("{}", "Connecting to real FIX market data stream".bright_white());
    println!("{}", "=".repeat(80).bright_blue());
    ...
    for line in env_content.lines() {
        if line.starts_with("BINANCE_API_KEY=") {
            ...
            private_key_env = line.strip_prefix("BINANCE_API_SECRET=").unwrap_or("").to_string();
        }
    }
    ...
    println!("DEBUG: Loaded API key from file: {}...{}", 
...
pub fn benchmark_fix_parsing(iterations: usize) -> (u64, u64, usize, usize) {
    ...
    let depth_fix = format!(
        ...
        SOH, SOH, SOH, SOH, SOH, 
        SOH, SOH, SOH, SOH,  // First bid
        SOH, SOH, SOH, SOH,  // Second bid
        SOH, SOH, SOH, SOH,  // Third bid
        SOH, SOH, SOH, SOH,  // First ask
        SOH, SOH, SOH, SOH,  // Second ask
        SOH, SOH, SOH, SOH   // Third ask
    );
    ...
    println!("@bookTicker: {} ns/msg ({} bytes)", book_ticker_ns, book_ticker_fix.len());
    println!("@depth:      {} ns/msg ({} bytes)", depth_ns, depth_fix.len());
    
    (book_ticker_ns, depth_ns, book_ticker_fix.len(), depth_fix.len())
}